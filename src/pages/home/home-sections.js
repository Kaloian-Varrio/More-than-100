...
export function createValueSection() {
  const values = [
    ['person-walking', 'Move more', 'Find enjoyable ways to add movement and build consistency.'],
    ['basket2', 'Eat better', 'Make nourishing choices that still fit real life and real routines.'],
    ['moon-stars', 'Restore well', 'Give sleep, recovery and quiet moments the attention they deserve.'],
    ['people', 'Connect deeply', 'Make room for relationships, shared time and belonging.'],
    ['wind', 'Reduce daily stress', 'Use simple practices to create more calm and perspective.'],
    ['repeat', 'Build lasting habits', 'Start small, learn what works and make progress sustainable.'],
  ];
  return `
    <section class="value-section section-space" id="why-it-matters" aria-labelledby="value-title"><div class="container"><div class="value-panel"><div class="row align-items-end g-4 mb-5"><div class="col-lg-8"><p class="section-eyebrow section-eyebrow--light mb-2">Why More Than 100</p><h2 class="section-title text-white mb-3" id="value-title">Healthier years are built in everyday moments.</h2><p class="value-copy mb-0">You do not need to transform everything overnight. Six connected areas can help you find one practical place to begin.</p></div><div class="col-lg-4"><div class="value-summary"><strong>6</strong><span>everyday themes for a healthier whole</span></div></div></div><div class="value-journey"><div class="row g-3">${values.map(([icon, title, description], index) => `<div class="col-md-6 col-xl-4"><article class="value-item h-100"><div class="value-item__top"><span class="value-item__icon"><i class="bi bi-${icon}" aria-hidden="true"></i></span><span class="value-item__number">${String(index + 1).padStart(2, '0')}</span></div><h3 class="h5 mt-4 mb-2">${title}</h3><p class="mb-0">${description}</p></article></div>`).join('')}</div></div></div></div></section>
